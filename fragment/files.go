...
func gatherFilesGit(cfg *Config, filesToInclude map[string]string) error {
    ...
    cmdOutput, err := runCommand(cfg.ProjectRoot, "git", "ls-files", "-c", "-o", "--exclude-standard", "-z")
    ...
    for _, relPath := range gitFiles {
        ...
        if absPath == cfg.AbsoluteOutputFile { continue } // Skip output
    ...
    for _, inputRoot := range cfg.AbsoluteInputDirs {
        ...
        err := filepath.WalkDir(inputRoot, func(absPath string, d fs.DirEntry, walkErr error) error {
            ...
            if walkErr != nil {
                ...
                if errors.Is(walkErr, fs.ErrPermission) { return nil }
            ...
            // Process Files
            absPath = filepath.Clean(absPath)
            if absPath == cfg.AbsoluteOutputFile { return nil } // Skip output
            if processedAbsPaths[absPath] { return nil } // De-duplicate
    ...
    for _, pattern := range excludePatterns {
        ...
        if strings.HasPrefix(pattern, "*.") && !strings.Contains(pattern, "/") {
            ...
            if strings.HasSuffix(relPath, suffix) {
                isExcluded = true; matchingPattern = pattern; break
            }
...
func isLikelyBinary(absPath string) (bool, error) {
    ...
    if err != nil { return false, fmt.Errorf("failed to open file %s: %w", absPath, err) }
    defer file.Close()
    buffer := make([]byte, 512)
    n, err := file.Read(buffer)
    if err != nil && err != io.EOF { return false, fmt.Errorf("failed to read file %s: %w", absPath, err) }
    if n == 0 { return false, nil }
    if bytes.Contains(buffer[:n], []byte{0}) { return true, nil }
    return false, nil
}
